...
func GetUserList(c *fiber.Ctx) error {
    ...
    // // return c.JSON(fiber.Map(films))
    fmt.Println("listing users... ")


    conn, err := config.PGconn()
    if err != nil {
        log.Fatal("error PGconn")
    }
    defer conn.Close(context.Background())

    var query_res string
    // err = conn.QueryRow(context.Background(), "select to_json(array_agg(u)) from tb_users u").Scan(&query_res)
    // err = conn.QueryRow(context.Background(), "select   jsonb_build_object('result',array_agg(u) ) from tb_users u ").Scan(&query_res)
    err = conn.QueryRow(context.Background(), " select jsonb_build_object('users', json_object_agg(user_id,to_json(u)) ) from tb_users u ").Scan(&query_res)
    // query_res  := `[{"user_id":1,"username":"kanibalv","cryptpwd":"$2a$06$IIzKesVMkZBBfJwZHyOJc.OKmDMMuIbpt3Hl.MSFJjPt74yPamjUG","email":"kanibalv@gmail.com","rol_id":1,"options":{"token_expiration_time": 8, "token_expiration_time_unit": "hour"},"user_enable":true},{"user_id":3,"username":"franco","cryptpwd":"$2a$06$OksxTBA6HzpSLC0Mqk8AjOHJOUg5oDsE.S8MpIVs1yxFjix1ow7Ou","email":"francovald esquintanilla@gmail.com","rol_id":2,"options":{"token_expiration_time": 8, "token_expiration_time_unit": "hour"},"user_enable":true}] `
    fmt.Printf("listing users... %v ", query_res)
    var result map[string]interface{}
    // var result []map[string]interface{}
    err = json.Unmarshal([]byte(query_res), &result)
    if err != nil {
        fmt.Printf("error json %v", err)
    }


    // return c.JSON(fiber.Map(users))
    // return c.JSON(query_res)
    // return c.JSON(query_res)
